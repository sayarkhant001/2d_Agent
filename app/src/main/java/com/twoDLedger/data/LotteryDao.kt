...
interface LotteryDao {
    ...
    @Transaction
    @Query("SELECT * FROM vouchers WHERE isArchived = 0 AND session = :session ORDER BY timestamp DESC")
    ...
    @Query("SELECT bets.number, SUM(bets.amount) as totalAmount FROM bets INNER JOIN vouchers ON bets.voucherId = vouchers.id WHERE vouchers.isArchived = 0 AND vouchers.session = :session GROUP BY bets.number ORDER BY totalAmount DESC")
    ...
    @Query("SELECT * FROM winning_history ORDER BY date DESC LIMIT 30")
    fun getWinningHistory(): Flow<List<WinningHistory>>

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertWinningHistory(history: List<WinningHistory>)

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertWinningHistorySingle(history: WinningHistory)
}
