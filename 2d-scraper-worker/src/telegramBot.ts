/**
 * Admin-Only Telegram Bot Engine for 2D Ledger & License Control
...
export interface SalePlan {
  id: string; // 'trial_2d' | 'one_year' | 'lifetime' | custom
  name: string;
  ...
  if (typeof document === 'string' && (document.startsWith('http://') || document.startsWith('https://'))) {
    try {
      const resp = await fetch(document, {
        headers: { 'User-Agent': 'Mozilla/5.0 (2D-Ledger-Release-Sync)' }
      });
      if (resp.ok) {
...
export async function getAppRelease(env: Env): Promise<AppReleaseRecord | null> {
  try {
    ...
    const res = await fetch(`${env.FIREBASE_DB_URL}/2d_app_release.json`, {
...
export async function isUserAdmin(senderId: number | string, env: Env): Promise<boolean> {
  ...
  try {
    const token = await getFirebaseToken(env);
    const res = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/admins/${sid}.json`, {
      headers: { 'Authorization': `Bearer ${token}` }
    });
    if (res.ok) {
      const data = await res.json();
...
export async function getAdmins(env: Env): Promise<AdminRecord[]> {
  ...
  try {
    const token = await getFirebaseToken(env);
    const res = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/admins.json`, {
      headers: { 'Authorization': `Bearer ${token}` }
    });
    if (res.ok) {
...
export async function addAdmin(env: Env, telegramId: string, name: string, addedBy: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/admins/${telegramId}.json`, {
      method: 'PUT',
      headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
...
export async function getReseller(senderId: number | string, env: Env): Promise<ResellerRecord | null> {
  ...
  try {
    const token = await getFirebaseToken(env);
    const res = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/resellers/${sid}.json`, {
      headers: { 'Authorization': `Bearer ${token}` }
    });
    if (res.ok) {
...
export async function getAllResellers(env: Env): Promise<Record<string, ResellerRecord>> {
  try {
    const token = await getFirebaseToken(env);
    const res = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/resellers.json`, {
      headers: { 'Authorization': `Bearer ${token}` }
    });
    if (res.ok) {
      const data = await res.json();
...
export async function saveReseller(env: Env, reseller: ResellerRecord): Promise<boolean> {
  ...
  try {
    const token = await getFirebaseToken(env);
    const res = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/resellers/${reseller.telegram_id}.json`, {
      method: 'PUT',
      headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
...
export async function removeReseller(env: Env, telegramId: string): Promise<boolean> {
  try {
    const token = await getFirebaseToken(env);
    const res = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/resellers/${telegramId}.json`, {
      method: 'DELETE',
      headers: { 'Authorization': `Bearer ${token}` }
    });
    return res.ok;
...
export async function settleResellerDue(
  ...
  try {
    ...
    await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/reseller_ledger/${resellerId}/${entryId}.json`, {
      method: 'PUT',
      headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({
        type: 'payment',
...
export async function getCommissionConfig(env: Env): Promise<CommissionConfig> {
  ...
  try {
    const token = await getFirebaseToken(env);
    const res = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/commissions.json`, {
      headers: { 'Authorization': `Bearer ${token}` }
    });
    if (res.ok) {
...
export async function getPaymentAccounts(env: Env): Promise<PaymentAccounts> {
  const defaults: PaymentAccounts = {
    wave: { number: '09778899001', name: '2D Ledger Admin' },
    kpay: { number: '09778899001', name: '2D Ledger Admin' }
  };
  try {
    const token = await getFirebaseToken(env);
    const res = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/payment_accounts.json`, {
      headers: { 'Authorization': `Bearer ${token}` }
    });
    if (res.ok) {
...
export async function recordResellerActivationDue(env: Env, keyRecord: LicenseKeyRecord): Promise<void> {
  ...
  try {
    const token = await getFirebaseToken(env);
    await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/keys/${keyRecord.cd_key}.json`, {
      method: 'PATCH',
      headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/reseller_ledger/${resellerId}/${keyRecord.cd_key}.json`, {
      method: 'PUT',
      headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({
        type: 'activation',
...
export async function getBuyerSession(chatId: number | string, env: Env): Promise<BuyerSession | null> {
  try {
    const token = await getFirebaseToken(env);
    const res = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/buyer_sessions/${chatId}.json`, {
      headers: { 'Authorization': `Bearer ${token}` }
    });
    if (res.ok) {
...
export async function getAdminState(chatId: number | string, env: Env): Promise<AdminState | null> {
  ...
  try {
    const token = await getFirebaseToken(env);
    const res = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/admin_states/${cid}.json`, {
      headers: { 'Authorization': `Bearer ${token}` }
    });
    if (res.ok) {
...
export async function setAdminState(chatId: number | string, state: AdminState | null, env: Env): Promise<boolean> {
  ...
  try {
    const token = await getFirebaseToken(env);
    if (!state) {
      await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/admin_states/${cid}.json`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      return true;
...
export async function getBuyerOrder(orderId: string, env: Env): Promise<BuyerOrder | null> {
  try {
    const token = await getFirebaseToken(env);
    const res = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/orders/${orderId}.json`, {
      headers: { 'Authorization': `Bearer ${token}` }
    });
    if (res.ok) {
...
export async function saveBuyerOrder(order: BuyerOrder, env: Env): Promise<boolean> {
  try {
    const token = await getFirebaseToken(env);
    const res = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/orders/${order.order_id}.json`, {
      method: 'PUT',
      headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
...
export async function getAutoApproveConfig(env: Env): Promise<boolean> {
  try {
    const token = await getFirebaseToken(env);
    const res = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/config/auto_approve.json`, {
      headers: { 'Authorization': `Bearer ${token}` }
    });
    if (res.ok) {
...
export async function getSalePlans(env: Env): Promise<Record<string, SalePlan>> {
  ...
  try {
    const token = await getFirebaseToken(env);
    const res = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/sale_plans.json`, {
      headers: { 'Authorization': `Bearer ${token}` }
    });
    if (res.ok) {
    ...
    await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/sale_plans.json`, {
      method: 'PUT',
      headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
...
export async function saveSalePlan(env: Env, plan: SalePlan): Promise<boolean> {
  try {
    const token = await getFirebaseToken(env);
    const res = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/sale_plans/${plan.id}.json`, {
      method: 'PUT',
      headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
...
export async function getAllLicenseKeys(env: Env): Promise<Record<string, LicenseKeyRecord>> {
  try {
    const token = await getFirebaseToken(env);
    const res = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/keys.json`, {
      headers: { 'Authorization': `Bearer ${token}` }
    });
    if (res.ok) {
      const data = await res.json();
...
export async function saveLicenseKey(env: Env, keyRecord: LicenseKeyRecord): Promise<boolean> {
  ...
  try {
    const token = await getFirebaseToken(env);
    const res = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/keys/${keyRecord.cd_key}.json`, {
      method: 'PUT',
      headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
...
export async function approveLicenseKey(env: Env, cdKey: string): Promise<{ ok: boolean; record?: LicenseKeyRecord }> {
  try {
    const token = await getFirebaseToken(env);
    const getRes = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/keys/${cdKey}.json`, {
      headers: { 'Authorization': `Bearer ${token}` }
    });
    ...
    const patchRes = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/keys/${cdKey}.json`, {
      method: 'PATCH',
      headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
    ...
    if (updatedRecord.device_fingerprint && updatedRecord.device_fingerprint !== 'approved_by_admin') {
      try {
        await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/devices/${updatedRecord.device_fingerprint}.json`, {
          method: 'PATCH',
          headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
...
export async function sendAppToUser(
  ...
  if (token) {
    try {
      const res = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/telegram_trials/${senderId}.json`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  if (!trialKey) {
    ...
    if (token) {
      ...
      await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/telegram_trials/${senderId}.json`, {
        method: 'PUT',
        headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const caption = `📱 <b>2D LEDGER (မြန်မာ 2D စာရင်းကိုင် ဆော့ဝဲလ် တရားဝင် APK)</b>\n\n` +
  ...
  if (release && release.file_id) {
    ...
    const sent = await sendTelegramDocument(
      ...
      appKb,
      'HTML',
      release.file_name || '2D_Ledger.apk'
    );
    ...
    const noAppMsg = `📲 <b>2D LEDGER စာရင်းကိုင် ဆော့ဝဲလ် တပ်ဆင်ရန်</b>\n\n` +
      `🔑 <b>သင်၏ ၃ ရက် အခမဲ့ စမ်းသပ်ခွင့် CD-Key:</b>\n` +
      `<code>${trialKey}</code>\n\n` +
...
export async function getAllRecipientIds(env: Env): Promise<Set<string>> {
  ...
  try {
    ...
    // 1. telegram_trials
    try {
      const res = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/telegram_trials.json`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
    ...
    // 2. buyer_orders
    try {
      const res = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/buyer_orders.json`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
...
export async function setWinningNumber(env: Env, num: string, authorId?: string): Promise<boolean> {
  try {
    ...
    const updates: Record<string, unknown> = {
      '2d_live_results/winning_number': num,
      '2d_live_results/first_prize': `000${num}`,
      '2d_live_results/twod': num.slice(-2),
      '2d_live_results/tuwt_set': tut.allTut,
      '2d_live_results/result_date': dateStr,
      '2d_live_results/is_final': true,
      '2d_live_results/updated_at': new Date().toISOString(),
      '2d_lottery_status/state': 'declared',
      '2d_lottery_config/mode': 'manual'
    };
...
export function getAdminBottomKeyboard(): ReplyKeyboardMarkup {
  return {
    keyboard: [
      ...
      [{ text: '🎯 ပေါက်မဲ' }, { text: '🇹🇭 2D Live ရလဒ်' }],
      [{ text: '🆔 ကျွန်ုပ်၏ ID' }]
    ],
...
export function getResellerBottomKeyboard(): ReplyKeyboardMarkup {
  return {
    keyboard: [
      ...
      [{ text: '🇹🇭 2D Live ရလဒ်' }, { text: '🆔 ကျွန်ုပ်၏ ID' }]
    ],
    resize_keyboard: true,
    is_persistent: true
  };
...
export function getResellerDashboardKeyboard(): InlineKeyboardMarkup {
  return {
    inline_keyboard: [
      [
        { text: '🎁 ၃ ရက် စမ်းသပ်ခွင့် (Trial)', callback_data: 'r_gen:trial_2d' }
      ],
      [
...
export function getResellerBulkKeyboard(): InlineKeyboardMarkup {
  return {
    inline_keyboard: [
      ...
      [
        { text: '🎁 ၃ ရက် Trial (၅ ခု)', callback_data: 'r_gen_b:trial_2d:5' },
        { text: '🎁 ၃ ရက် Trial (၁၀ ခု)', callback_data: 'r_gen_b:trial_2d:10' }
      ],
      [
...
export function buildResellerDashboardMessage(reseller: ResellerRecord): { text: string; keyboard: InlineKeyboardMarkup } {
  const text = `💼 <b>2D LEDGER ကိုယ်စားလှယ် ဒက်ရှ်ဘုတ် (RESELLER PANEL)</b>\n\n` +
...
export function getKeyGenKeyboard(plans?: Record<string, SalePlan>): InlineKeyboardMarkup {
  return {
    inline_keyboard: [
      ...
      [
        { text: '🎁 ၃ ရက် Trial (၁ ခု)', callback_data: 'gen_b:trial_2d:1' },
        { text: '🎁 ၃ ရက် Trial (၅ ခု)', callback_data: 'gen_b:trial_2d:5' }
      ],
      [
        { text: '⬅️ ပင်မ မီနူးသို့ ပြန်သွားမည်', callback_data: 'm_main' }
      ]
...
export function buildPlansDashboardMessage(plans: Record<string, SalePlan>): string {
  let text = `🏷️ <b>2D LEDGER အရောင်း အစီအစဉ်များ (SALE PLANS)</b>\n\n` +
...
export async function buildAdminDashboardMessage(env: Env): Promise<{ text: string; keyboard: InlineKeyboardMarkup }> {
  ...
  const text = `👑 <b>2D LEDGER စီမံခန့်ခွဲမှု စင်တာ (ADMIN CONTROL)</b>\n\n` +
...
export async function buildSalesReportMessage(env: Env): Promise<string> {
  ...
  return `📊 <b>2D LEDGER အရောင်းနှင့် အသုံးပြုမှု အစီရင်ခံစာ</b>\n\n` +
...
export async function handleTelegramWebhook(request: Request, env: Env): Promise<Response> {
  ...
  if (update.callback_query) {
    ...
    // 1.2 Reseller Key Generation Callbacks
      ...
      const planId = data.split(':')[1];
      if (planId !== 'trial_2d' && planId !== 'one_year' && planId !== 'lifetime') {
    ...
    // 1.4 Admin Buyer Order Verification
      ...
      const buyerMsg = `🎉 <b>သင့်ငွေလွှဲ အတည်ပြုပြီးပါပြီ!</b>\n\n` +
        `<b>2D LEDGER စာရင်းကိုင် ဆော့ဝဲလ်</b> လိုင်စင် အသုံးပြုခွင့်ကုတ်နံပါတ် ရရှိပါပြီ:\n\n` +
    ...
    // 1.5 Activation Approvals (Admin OR Key Reseller)
      ...
      try {
        const token = await getFirebaseToken(env);
        const kRes = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/keys/${keyToApprove}.json`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
      ...
      try {
        const token = await getFirebaseToken(env);
        const kRes = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/keys/${keyToReject}.json`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        if (kRes.ok) keyRecord = await kRes.json() as LicenseKeyRecord;
      } catch (_) {}
    ...
    // Public Live Refresh callback
      ...
      await answerCallbackQuery(env, cq.id, '🔄 2D Live ရလဒ် စစ်ဆေးပြီးပါပြီ');
      if (glo) {
    ...
    // Public Buy Menu callback
      ...
      const buyMsg = `💎 <b>2D LEDGER လိုင်စင် အစီအစဉ်များ ဝယ်ယူရန်</b>\n\n` +
    ...
    // Public Help callback
      ...
      await answerCallbackQuery(env, cq.id);
      const helpText = `📖 <b>2D LEDGER စာရင်းကိုင် ဆော့ဝဲလ် အသုံးပြုနည်း လမ်းညွှန်</b>\n\n` +
    ...
    // Forwarded User Action: fwd_add_res:<targetId>:<encName>
      ...
      if (ok) {
        ...
        await sendTelegramMessage(env, targetId,
          `🎉 <b>မင်္ဂလာပါ ${targetName}! သင့်အား 2D LEDGER ကိုယ်စားလှယ် (Reseller) အဖြစ် စာရင်းသွင်းလိုက်ပါပြီ။</b>\n\n` +
    ...
    // Forwarded User Action: fwd_add_adm:<targetId>:<encName>
      ...
      if (ok) {
        ...
        await sendTelegramMessage(env, targetId,
          `🎉 <b>ဂုဏ်ယူပါသည်! သင့်အား 2D LEDGER စနစ်၏ Admin အဖြစ် ခန့်အပ်လိုက်ပါပြီ။</b>`,
    ...
    // Declare Winner Menu
      ...
      const text = `🎯 <b>2D ပေါက်မဲ သတ်မှတ်ခြင်း (Winner Declaration)</b>\n\n` +
    ...
    // Batch Menu: m_batch
      ...
      let nextDrawDate = '';
      try {
        const res = await fetch(`${env.FIREBASE_DB_URL}/2d_lottery_config/current_batch.json`);
        if (res.ok) b = parseInt(await res.json() || '1', 10) || 1;
        const liveRes = await fetch(`${env.FIREBASE_DB_URL}/2d_live_results.json`);
        if (liveRes.ok) {
      ...
      const text = `📦 <b>2D ထီထွက်ရက်စွဲ နှင့် အပတ်စဉ် အချက်အလက်</b>\n\n` +
    ...
    // Batch Increment: b_inc
      ...
      try {
        const token = await getFirebaseToken(env);
        await fetch(`${env.FIREBASE_DB_URL}/2d_lottery_config/current_batch.json`, {
          method: 'PUT',
          headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
  ...
  if (update.message && update.message.document) {
    ...
    if (userIsAdmin) {
      ...
      if (isApk) {
        ...
        const releaseNotes = msg.caption?.replace(/^\/\w+\s*/, '').trim() || '2D Ledger နောက်ဆုံးထွက် Official APK ဗားရှင်း';
  ...
  if (update.message && update.message.text) {
    ...
    if (userIsAdmin) {
      ...
      if (adminState) {
        ...
        if (adminState.action === 'awaiting_wave') {
          const parts = text.split(/\s+/);
          const num = parts[0];
          const name = parts.slice(1).join(' ') || '2D Ledger Admin';
          if (!num) {
        ...
        if (adminState.action === 'awaiting_winner') {
          ...
          await sendTelegramMessage(env, chatId,
            `🎯 <b>2D ပေါက်ဂဏန်း အောင်မြင်စွာ ကြေညာပြီးပါပြီ</b>\n\n` +
    ...
    if (text === '🇹🇭 2D Live ရလဒ်' || text.startsWith('/live') || text.startsWith('/2d') || text.startsWith('/result')) {
      const glo = await fetchFromGloLottery();
      if (glo) {
        const liveKb: InlineKeyboardMarkup = {
          inline_keyboard: [
    ...
    if (text === '🔢 တွတ် ဂဏန်းများ') {
      ...
      await sendTelegramMessage(env, chatId,
        `🔢 <b>မြန်မာ 2D တွတ် (Tut) ဂဏန်းများ တွက်ချက်ခြင်း</b>\n\n` +
    ...
    if (text === '❓ အကူအညီ' || text === '/help') {
      const helpText = `📖 <b>2D LEDGER စာရင်းကိုင် ဆော့ဝဲလ် အသုံးပြုနည်း လမ်းညွှန်</b>\n\n` +
        ...
        `• <b>[🇹🇭 2D Live ရလဒ်]</b> ခလုတ်ဖြင့် တရားဝင် ထိုင်း GLO ပေါက်မဲကို တိုက်ရိုက် ကြည့်ရှုနိုင်ပါသည်။`;
    ...
    if (text.startsWith('/batch')) {
      ...
      const msg = `📦 <b>2D ထီထွက်ရက်စွဲ နှင့် အပတ်စဉ်</b>\n\n` +
        (drawDate ? `📅 <b>နောက်ဆုံးထွက်ရက်:</b> <code>${drawDate}</code>\n` : '') +
        (nextDrawDate ? `⏭️ <b>နောက်ထွက်မည့်ရက်:</b> <code>${nextDrawDate}</code>\n` : '') +
        `🔢 <b>စနစ် သတ်မှတ် အကြိမ်:</b> #${b}\n\n` +
        `💡 <i>မှတ်ချက်: အက်ပ်အသုံးပြုသူများသည် မိမိဖုန်းအလိုက် အကြိမ်များကို သီးခြားစီမံနိုင်ပါသည်။</i>`;
    ...
    if (!userIsAdmin && !reseller) {
      if (text === '/menu' || text === '👋 မီနူး') {
        ...
        const oneYearPrice = plans.one_year?.price ? plans.one_year.price.toLocaleString() : '180,000';
        const lifetimePrice = plans.lifetime?.price ? plans.lifetime.price.toLocaleString() : '45,000';
        const menuMsg = `👋 <b>မင်္ဂလာပါ! 2D LEDGER စနစ်မှ ကြိုဆိုပါသည်</b>\n\n` +
      ...
      if (
        ...
        await sendTelegramMessage(
          ...
          `ဤ Telegram Bot သည် <b>2D Ledger စနစ် စီမံခန့်ခွဲသူ (Admin) နှင့် ကိုယ်စားလှယ်</b> များ သီးသန့် အသုံးပြုရန် ဖြစ်ပါသည်။\n\n` +
      ...
      await sendTelegramMessage(env, chatId, '👋 <b>2D LEDGER စနစ်မှ ကြိုဆိုပါသည်!</b>', getBuyerBottomKeyboard());
      ...
      if (text.startsWith('/start download')) {
        ...
        if (release && release.file_id) {
          ...
          const caption = `📱 <b>2D LEDGER စာရင်းကိုင် ဆော့ဝဲလ် (Official APK)</b>\n\n` +
      ...
      if (!existingTrial) {
        ...
        const welcomeText = `🎁 <b>မင်္ဂလာပါ ${msg.from?.first_name || 'မိတ်ဆွေ'}</b>\n\n` +
          `<b>2D LEDGER စာရင်းကိုင် ဆော့ဝဲလ်</b> မှ ကြိုဆိုပါသည်!\n` +
        ...
        const keyDataRes = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/keys/${existingTrial.cd_key}.json`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
    ...
    if (reseller && !userIsAdmin) {
      if (text === '/start' || text === '/reseller' || text === '/menu' || text === '💼 ဒက်ရှ်ဘုတ်') {
        await sendTelegramMessage(env, chatId, '💼 <b>2D LEDGER ကိုယ်စားလှယ် ဒက်ရှ်ဘုတ်</b>', getResellerBottomKeyboard());
        const dash = buildResellerDashboardMessage(reseller);
        await sendTelegramMessage(env, chatId, dash.text, dash.keyboard);
        return new Response('ok');
      }
      ...
      if (text === '🎁 ၃ ရက် Trial' || text === '📅 ၁ နှစ် လိုင်စင်' || text === '💎 တစ်သက်တာ လိုင်စင်') {
        ...
        if (text === '🎁 ၃ ရက် Trial') planKey = 'trial_2d';
      ...
      if (text.startsWith('/gen')) {
        ...
        if (parts.length === 1) {
          const genText = `➕ <b>ကိုယ်စားလှယ် ကုတ်အသစ် ထုတ်ယူရန် စီမံချက် ရွေးချယ်ပါ</b>\n\n` +
            ...
            `• <code>/gen trial_2d</code> - ၃ ရက် စမ်းသပ်ခွင့် (Free Trial)\n` +
        ...
        } else if (arg1 === 'trial_2d' || arg1 === 'trial' || arg1 === '2d' || arg1 === '3') {
          targetPlan = plans.trial_2d || getDefaultSalePlans().trial_2d;
        } else if (plans[arg1] && (arg1 === 'one_year' || arg1 === 'lifetime' || arg1 === 'trial_2d')) {
          targetPlan = plans[arg1];
        }

        if (!targetPlan) {
          await sendTelegramMessage(env, chatId, '⚠️ ကိုယ်စားလှယ်များ အနေဖြင့် trial_2d, one_year နှင့် lifetime ကုတ်များကိုသာ ထုတ်ယူနိုင်ပါသည်။');
          return new Response('ok');
        }
    ...
    if (text === '/start' || text === '/menu' || text === '/admin' || text === '📊 ပင်မ ဒက်ရှ်ဘုတ်') {
      await sendTelegramMessage(env, chatId, '👑 <b>2D LEDGER စီမံခန့်ခွဲသူ (Admin) စနစ်</b>', getAdminBottomKeyboard());
      const dash = await buildAdminDashboardMessage(env);
      await sendTelegramMessage(env, chatId, dash.text, dash.keyboard);
      return new Response('ok');
    }
    ...
    if (text.startsWith('/addadmin')) {
      ...
      if (ok) {
        await sendTelegramMessage(env, chatId, `✅ <b>Admin အသစ် [${targetName}] (ID: <code>${targetId}</code>) အား အောင်မြင်စွာ ခန့်အပ်လိုက်ပါပြီ။</b>`);
        await sendTelegramMessage(env, targetId, `🎉 <b>ဂုဏ်ယူပါသည်! သင့်အား 2D LEDGER စနစ်၏ Admin အဖြစ် ခန့်အပ်လိုက်ပါပြီ။</b>\n\nစီမံခန့်ခွဲမှု မီနူး ဖွင့်ရန် <code>/admin</code> ဟု စာပို့ပါ`);
      } else {
        await sendTelegramMessage(env, chatId, '❌ Admin ထည့်သွင်းခြင်း မအောင်မြင်ပါ');
      }
      return new Response('ok');
    ...
    if (text.startsWith('/setwave')) {
      const parts = text.split(/\s+/);
      const num = parts.length > 1 ? parts[1].trim() : '';
      const name = parts.length > 2 ? parts.slice(2).join(' ').trim() : '2D Ledger Admin';
      if (!num) {
    ...
    // Command: /setwinner <2-digit number>
    ...
    if (text.startsWith('/setappid')) {
      ...
      const notes = parts.length > 3 ? parts.slice(3).join(' ').trim() : '2D Ledger APK နောက်ဆုံးထွက် ဗားရှင်း';
      ...
      const release: AppReleaseRecord = {
        file_id: fileId,
        file_name: `2D_Ledger_${version}.apk`,
        file_size: 9284449,
    ...
    if (text.startsWith('/setapplink')) {
      ...
      const version = parts.length > 2 ? parts[2].trim() : 'v1.0.100';
      const notes = parts.length > 3 ? parts.slice(3).join(' ').trim() : '2D Ledger Direct APK Download';
    ...
    if (text.startsWith('/broadcast') || text.startsWith('/announce')) {
      ...
      if (isAppUpdate || !customMsg) {
        ...
        await broadcastTextMessage(env, chatId, `📢 <b>[2D LEDGER အသိပေးချက်]</b>\n\n${customMsg}`);
      }
      return new Response('ok');
    }
