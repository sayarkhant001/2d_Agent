 ...
 * Fetches Myanmar 2D results directly from the official Government Lottery Office (GLO) Thailand API
 * (https://www.glo.or.th/api/lottery/getLatestLottery)
 * and pushes to Firebase (authenticated via service account).
 *
 * Thai 2D winning number = last 3 digits of the official 1st prize (รางวัลที่ 1).
 */
...
export default {
  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === '/health' || url.pathname === '/ping') {
      return new Response(JSON.stringify({ status: 'ok', service: '2d-scraper-worker' }), {
        headers: corsHeaders
      });
    }
    ...
    if (url.pathname === '/broadcast' || url.pathname === '/broadcast-update') {
      ...
      const defaultAnnouncement =
        `🚀 <b>2D LEDGER စနစ် အဆင့်မြှင့်တင်မှု အသစ် ထွက်ရှိပါပြီ!</b>\n\n` +
        `✨ <b>အဓိက ပြောင်းလဲမှုများ:</b>\n` +
        `• 📱 Android App: လက်တွေ့ဆန်သော 2D ခလုတ်ဒီဇိုင်း (Physical Tactile Keypad) နှင့် Haptic တုန်ခါမှု တုံ့ပြန်စနစ်\n` +
    ...
    if ((url.pathname === '/api/app/publish' || url.pathname === '/app/publish') && request.method === 'POST') {
      try {
        ...
        const releaseRecord: AppReleaseRecord = {
          file_id: fileId,
          file_name: body.file_name || `2D_Ledger_${versionName}.apk`,
    ...
    if ((url.pathname === '/api/app/sync-telegram' || url.pathname === '/app/sync-telegram') && request.method === 'POST') {
      try {
        ...
        const tgRes = await sendTelegramDocument(
          ...
          `📦 <b>[Official 2D Ledger Production APK]</b>\n\n` +
          ...
          undefined,
          'HTML',
          release.file_name || `2D_Ledger_${release.version_name}.apk`
        );
    ...
    if (url.pathname === '/broadcast-winner' || url.pathname === '/test-broadcast') {
      try {
        ...
        const crRes = await fetch(`${env.FIREBASE_DB_URL}/2d_live_results.json`, {
    ...
    if (url.pathname === '/') {
      return new Response(JSON.stringify({
        service: '2d-scraper-worker & telegram-bot & license-api',
        status: 'online',
        endpoints: [
          '/health',
          '/latest-glo',
...
export async function processDraw(env: Env, options?: ProcessDrawOptions) {
  ...
  if (!result) {
    if (isForce) {
      await sendTelegramAlert(env, '⚠️ No official Thai 2D result available from GLO API.');
    } else {
  ...
  try {
    const modeRes = await fetch(`${env.FIREBASE_DB_URL}/2d_lottery_config/mode.json`, {
      headers: { 'Authorization': `Bearer ${token}` }
    });
    if (modeRes.ok) {
  ...
  try {
    const cr = await fetch(`${env.FIREBASE_DB_URL}/2d_live_results.json`, {
      headers: { 'Authorization': `Bearer ${token}` }
    });
  ...
  const updates: Record<string, unknown> = {
    ...
    '2d_live_results/result_date':      result.date,
    '2d_live_results/result_time':      result.session,
    '2d_live_results/is_final':         result.isFinal,
...
export async function signLicenseJwt(payload: Record<string, unknown>, secret = '2d-ledger-jwt-secret-2026'): Promise<string> {
...
export async function handleLicenseActivate(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    const isTrial = keyData.duration === 'trial' || keyData.plan_id === 'trial_2d';
...
export async function handleLicenseRestore(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    // 1. Direct device mapping lookup in /2d_licenses/devices/${device_fingerprint}.json
    let boundKey: string | null = null;
    try {
      const devRes = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/devices/${device_fingerprint}.json`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (devRes.ok) {
    ...
    // 2. If no direct match, scan /2d_licenses/keys.json for active key matching this device
    if (!keyData || keyData.device_fingerprint !== device_fingerprint || keyData.status !== 'active' || (keyData.expires_at && now >= keyData.expires_at)) {
      try {
        const allKeysRes = await fetch(`${env.FIREBASE_DB_URL}/2d_licenses/keys.json`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        if (allKeysRes.ok) {
